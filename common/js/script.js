...
function updatePlayerPosition() {
  const positions = ['calc(33% - 25px)', 'calc(50% - 25px)', 'calc(66% - 25px)'];
  gp.style.left = positions[position];
}
...
function addPit() {
  ...
  // 장애물이 무작위 위치에 나오도록 설정
  const pitPosition = Math.floor(Math.random() * 3); // 0: 왼쪽, 1: 중간, 2: 오른쪽
  const positions = ['calc(20% - 50px)', 'calc(50% - 50px)', 'calc(80% - 50px)'];
  p.style.left = positions[pitPosition];
...
function checkForFail() {
  ...
  if (hit && hit.classList.contains('pit')) {
